node {
    ...
    env.IMAGE = 'lacina15/lass-amazon-app'

    stage('Clone repository') {
             git branch: 'main', url: 'https://github.com/kone15229/argocd-amazon-manifest.git'  
    }
            ...
            script {
                catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                    withCredentials([usernamePassword(credentialsId: 'jenkins-token', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                        ...
                        sh "git config user.email lacinakone88@gmail.com"
                        sh "git config user.name lacina"
